      ...
      {modules.map((module) => {
        ...
        return (
          <Link key={module.id} href={module.href} className="group block cursor-pointer">
            <div
              className={`${dashboardCardClass} h-full transition-all duration-200 hover:border-[var(--color-accent-custom,#22C55E)]/30 hover:shadow-md`}
            >
              <div className="border-b border-gray-100 px-4 py-3.5 dark:border-border">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    {Icon ? (
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--color-accent-custom,#22C55E)]/10">
                        <Icon className="h-[18px] w-[18px] text-[var(--color-accent-custom,#22C55E)]" />
                      </div>
                    ) : null}
                  ...
                  <ChevronRight className="h-4 w-4 shrink-0 text-gray-400 transition-colors group-hover:text-[var(--color-accent-custom,#22C55E)]" />
                </div>
              </div>
                          ...
                          {typeof stat.change === "number" ? (
                            <span
                              className={`inline-flex shrink-0 items-center gap-0.5 text-[11px] font-medium ${
                                stat.change >= 0 ? "text-[var(--color-accent-custom,#22C55E)]" : "text-red-500"
                              }`}
                            >
                              {stat.change >= 0 ? (
