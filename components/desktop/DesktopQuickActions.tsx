  ...
  return (
    <div className="fixed bottom-10 right-5 z-[180] flex flex-col items-end gap-2">
        ...
        <div className="flex flex-col gap-1.5 mb-1 animate-in fade-in slide-in-from-bottom-2 duration-150">
          {actions.map((a) => (
            <button
              ...
              className="flex items-center gap-2 rounded-full bg-[#0f172a] text-white text-xs font-medium pl-3 pr-3.5 py-2 shadow-lg border border-white/10 hover:border-[#4A5D7A]/40"
            >
              <a.icon className="h-3.5 w-3.5 text-[#4A5D7A]" />
              {a.label}
            </button>
          ))}
        </div>
      ...
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="h-12 w-12 rounded-full bg-gradient-to-br from-[#2E3E52] to-[#4A5D7A] text-white shadow-xl shadow-green-900/30 grid place-items-center hover:scale-105 transition-transform"
      >
