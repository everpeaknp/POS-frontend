...
const paymentStyles = {
  ...
  upi: "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-400",
...
export function PosPaymentMethodBadge({
  ...
  method: "cash" | "card" | "upi" | "credit" | string;
}) {
  ...
  return (
    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium uppercase ${style}`}>
      {method}
    </span>
  );
}
