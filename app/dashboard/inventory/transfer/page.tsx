...
export default function StockTransferPage() {
  ...
  const { data: movements, loading: loadingMovements, refetch } = useApi(
    () => inventoryApi.movements.list({ movement_type: 'transfer', page_size: 500 }),
    { immediate: true }
  );
  ...
  const transfers = (movements?.data?.results || []).filter(
    (t: { movement_type: string; from_warehouse: number | null; warehouse: number }) =>
      t.movement_type === 'transfer' &&
      t.from_warehouse != null &&
      t.warehouse === t.from_warehouse
  );
              ...
              <tbody className="divide-y divide-gray-50">
                {paginatedTransfers.map((t: any) => (
                  <tr key={t.id} className="hover:bg-gray-50/50">
                    <td className="px-4 py-3 font-medium text-[var(--color-accent-custom,#22C55E)]">#{t.id}</td>
