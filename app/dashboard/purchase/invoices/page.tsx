    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex-1 p-6 space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button size="sm" className="h-9 bg-[var(--color-accent-custom,#22C55E)] hover:bg-[var(--color-accent-custom-600,#16A34A)] text-white gap-1.5" onClick={() => router.push("/dashboard/purchase/invoices/new")}>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((inv) => (
                    <tr key={inv.id} className={`hover:bg-gray-50/50 transition-colors ${inv.status === "Overdue" ? "border-l-2 border-red-400" : ""}`}>
                      <td className="px-4 py-3 font-medium text-[var(--color-accent-custom,#22C55E)] cursor-pointer hover:underline" onClick={() => router.push(`/dashboard/purchase/invoices/${inv.id}`)}>{inv.invoice_number}</td>
