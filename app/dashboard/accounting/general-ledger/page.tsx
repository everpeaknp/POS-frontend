...
export default function GeneralLedgerPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Failed to generate ledger');
    } finally {
      setGenerating(false);
    }
  };
      ...
      <div className="flex-1 p-6 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
            <div className="sm:col-span-2">
              <Label className="text-sm mb-1.5 block">Account <span className="text-red-500">*</span></Label>
              <select 
                value={accountId} 
                onChange={(e) => {
                ...
                className="w-full h-9 text-sm border border-gray-200 rounded-md px-3 bg-white focus:outline-none focus:border-[#22C55E]"
                disabled={generating}
              >
          ...
          <>
            <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm flex flex-wrap items-center gap-4">
              <div>
                ...
                <p className="font-semibold text-gray-800">{account.code} — {account.name}</p>
              </div>
              <AccountTypeBadge type={account.type} />
              <div className="ml-auto">
                <p className="text-xs text-gray-500">Opening Balance</p>
                <p className="font-semibold text-gray-800">
                  {ledgerEntries[0] ? fmt(ledgerEntries[0].balance - ledgerEntries[0].debit + ledgerEntries[0].credit) : fmt(0)}
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {ledgerEntries.map((entry, i) => (
                    <tr key={i} className="hover:bg-gray-50/50">
                      <td className="px-4 py-3 text-gray-600">{new Date(entry.date).toLocaleDateString('en-GB')}</td>
                ...
                <tfoot className="bg-gray-50 border-t border-gray-200">
                  <tr>
                    <td colSpan={3} className="px-4 py-3 text-xs font-semibold text-gray-600">Totals</td>
        ...
        {generated && account && ledgerEntries.length === 0 && (
          <div className="bg-white rounded-xl border border-gray-100 p-12 shadow-sm text-center">
            <p className="text-gray-400 text-sm">No transactions found for this account in the selected date range.</p>
          </div>
        )}

        {!generated && (
          <div className="bg-white rounded-xl border border-gray-100 p-12 shadow-sm text-center">
            <p className="text-gray-400 text-sm">Select an account and date range, then click Generate to view the ledger.</p>
          </div>
        )}
      </div>
    </div>
