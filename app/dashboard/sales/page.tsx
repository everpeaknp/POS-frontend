...
export default function SalesDashboardPage() {
  ...
  const subtitle = `${workspaceName} · ${t('sales.sales_overview_and_analytics')}`;
            ...
            >
              ...
              <div
                className={`inline-flex items-center gap-0.5 text-xs font-medium mt-1 ${
                  change >= 0 ? "text-slate-600" : "text-red-500"
                }`}
              >
                {change >= 0 ? (
        ...
        {creditOverview && (
          <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/sales/customers" className="text-xs text-[#4A5D7A] hover:underline">
                View customers
              </Link>
            </div>
                ...
                {((creditOverview as { customers: Array<{ id: string; name: string; current_balance: number }> }).customers)
                  ...
                  .map((c) => (
                    <li key={c.id} className="flex justify-between py-2 text-sm">
                      <Link href={`/dashboard/sales/customers/${c.id}`} className="hover:text-[#4A5D7A]">
                        {c.name}
                      </Link>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {recentOrders.map((o) => (
                  ...
                  >
                    <td className="px-4 py-2.5 font-mono text-xs text-[#4A5D7A]">{o.order_number || o.id}</td>
            ...
            <div className="space-y-3">
              {recentCustomers.map((c) => (
                <div key={c.email} className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-[#4A5D7A] text-white text-xs font-semibold flex items-center justify-center shrink-0">
                    {c.initials}
                  </div>
